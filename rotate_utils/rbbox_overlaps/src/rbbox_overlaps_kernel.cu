...
at::Tensor rbbox_overlaps(const at::Tensor boxes, const at::Tensor query_boxes, const int gpu_device_id) {
  ...
  AT_ASSERTM(query_boxes.type().is_cuda(), "query_boxes must be a CUDA tensor");
  ...
  scalar_t* query_boxes_dev = query_boxes.data<scalar_t>();

  THCState *state = at::globalContext().lazyInitCUDA(); // TODO replace with getTHCState

  float* overlaps = NULL;
  overlaps = (float*) THCudaMalloc(state, boxes_num * query_boxes_num * sizeof(float));

  _overlaps(overlaps, boxes_dev, query_boxes_dev, boxes_num, query_boxes_num, gpu_device_id);
  THCudaFree(state, overlaps); //free the cuda mem

  at::Tensor output = at::from_blob(overlaps, /*size=*/{boxes_num, query_boxes_num});

  return std::move(output.cuda());
}
