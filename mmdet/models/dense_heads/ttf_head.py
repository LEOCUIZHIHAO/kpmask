...
from mmdet.core.anchor.utils import calc_region
...
class TTFHead(AnchorHead):
    ...
    def get_bboxes(self,
        ...
        topk = getattr(self.cfg, 'max_per_img', 100)
        # (batch, topk)
        ...
        result_list = []
        score_thr = getattr(self.cfg, 'score_thr', 0.01)
        for batch_i in range(bboxes.shape[0]):
            ...
            if rescale:
                ...
                bboxes_per_img /= bboxes_per_img.new_tensor(scale_factor) ## WARNING: 512*512 -> original pic size
    ...
    def target_single_image(self, gt_boxes, gt_labels, feat_shape):
        ...
        for k in range(boxes_ind.shape[0]):
            ...
            if False:
                import mmcv
                print("cls_id", gt_labels[k])
                l = np.array([gt_labels[k]])
                bbox = np.ones((1,4))
                _heatmap = heatmap.detach().cpu().numpy()
                _hm = np.expand_dims(_heatmap[gt_labels[k]], -1)
                _hm = np.repeat(_hm, 3, -1)
                mmcv.imshow_det_bboxes(_hm * 255, bbox.reshape(1,-1), l, show_confidence=False)
