...
class BitmapMasks(BaseInstanceMasks):
    ...
    def shiftscalerotate(self, angle, scale, shift):
        """shift scale and rotate mask.

        Args:
            angle (int): ''
            scale (int): ''
            shift (list): '[left_shift, right_shifr]'
        Returns:
            BitmapMasks: the flipped masks
        """
        if len(self.masks) == 0:
            ssr_masks = self.masks
        else:
            ssr_masks = np.stack([
                mmcv.imshiftscalerotate(mask, angle, scale=scale,
                                        shift_x=shift[0], shift_y=shift[1])
                for mask in self.masks
            ])
        return BitmapMasks(ssr_masks, self.height, self.width)
